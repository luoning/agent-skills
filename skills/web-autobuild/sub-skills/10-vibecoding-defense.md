...
1. **正向覆盖 (Forward Coverage)**：
   * 所有在 `.extracted_facts.json` 中定义的核心数据字段（如金额、物理指标、服务参数、核心代号值）必须 100% 出现在生成的 HTML/JS 页面中。
2. **反向防幻觉 (Reverse Anti-Hallucination)**：
   * 任何在 HTML/JS 中出现的**数值常量、特定价格、规格参数代码**，必须能在 `.extracted_facts.json` 中找到对应的事实映射。
...
2. 检测是否有未定义的事实数据泄露到网页中（例如，虚假数值规格、未声明的价格参数等）。

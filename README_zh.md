...
| **无源事实/血缘断裂** | 页面硬编码数据且无来源追溯 | **数据血缘网关**：要求页面数值标明 `data-fact-source` 路径，阻断未声明来源 of 无源常数。 | **100% 可追溯** |
...
└── skills/
    ...
    ├── skill-governance/
    │   ├── SKILL.md               <-- [路由主控] 中央流程化编排器与子技能动态分发器
    │   └── sub-skills/
    │       ├── workflow-contract.md <-- 畅想、观点、方案三级内容沙盒物理隔离契约
    │       ├── url-extractor.md     <-- 基于 POE 模型对网页进行用户意图与核心摩擦深度挖掘
    │       ├── static-auditor.md    <-- 自动检验技能元 Schema 规范并拦截本地绝对盘符泄露
    │       ├── scenario-tester.md   <-- 强制所有场景测试继承并执行 BaseScenarioTest 四阶段生命周期
    │       └── skill-generator.md   <-- 技能的自动化编写、配置注册与双模式审计规范
...
用于审查 AI 编写的技能规范自身，防止包含私有项目数据 and 不便移植的配置：
